...
test("diagnostics distinguish changed values and log only on A",function()
 local b=Mock.new();b:tap("DOWN");b:tap("DOWN");b:tap("DOWN");b:tap("A")
 b:tick(100);b:tick(500);assert(has(b,"not Hz") and has(b,"peak"))
 local n=#b.logs;b:tick(500);assert(#b.logs==n);b:tap("A");assert(#b.logs==n+1)
 assert(b.logs[#b.logs]:find("free=",1,true));assert(b.enables==0)
end)
