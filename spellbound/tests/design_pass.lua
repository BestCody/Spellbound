...
test("Teach preloading deletes and rebuilds the UI",function()
 ...
 assert(b:state().phase=="train_select" and #b.widgets==9)
 ...
 assert(log_has(b,"MEM teach-after-gesture"))
 assert(log_has(b,"MEM teach-after-ui-rebuild") and log_has(b,"widgets=9"))
end)
test("duel preloading deletes UI before engine and radio",function()
 local b=Mock.new();b:tap("A")
 assert(b:state().phase=="lobby" and #b.widgets==9)
 ...
 assert(log_has(b,"MEM duel-after-ui-rebuild") and log_has(b,"widgets=9"))
end)
