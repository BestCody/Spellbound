...
test("duel preloading deletes UI before engine and radio",function()
 ...
 assert(b:state().phase=="lobby" and #b.widgets==5)
 ...
 assert(log_has(b,"MEM duel-after-ui-rebuild") and log_has(b,"widgets=5"))
end)
...
test("low-memory UI uses five bounded widgets",function()
