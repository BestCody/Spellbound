...
test("failed Bluetooth initialization is not retried in the same session",function()
 local b=Mock.new({production=true,radio=false});b:tap("A");b:tap("A")
 assert(b.enables==1,"Repeated controller initialization can leak or fragment heap")
end)
