...
function S.network_tick(now)
  ...
  elseif phase=="waiting" or phase=="starting" or phase=="joining" then
    ...
    if now>=S.next_tx then
      S.transmit(phase=="waiting" and "I" or (phase=="starting" and "S" or "J"),
        phase=="waiting" and S.peer or nil)
      S.next_tx=now+600
    end
  elseif phase=="duel" or phase=="result" then
    if phase=="duel" and now-S.last_rx>6000 then S.end_link("Link lost - match cancelled");return end
    ...
    elseif S.role=="guest" then
      if now-(S.last_state_tx or 0)>=750 then S.transmit("P");S.last_state_tx=now end
...
function S.net_render(now,shown)
  ...
  elseif phase=="waiting" then return "Invitation queued.\nWaiting for opponent.\n\nB cancels"
  elseif phase=="starting" or phase=="joining" then return "Synchronizing...\n\nB cancels" end
