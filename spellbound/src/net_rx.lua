...
return function(S)
  function S.receive(mac,rssi,payload)
    ...
    if k=="Q" and d=="" and S.phase=="offer" and S.invite.peer==from and S.invite.sid==s then
    ...
    elseif k=="K" and d=="" and S.role=="host" and S.phase=="starting" then
      S.last_rx=now;S.phase="duel";S.match.started=now;S.send_state(now)
    ...
    elseif k=="T" and S.role=="guest" and (S.phase=="joining" or S.phase=="duel" or S.phase=="result") then
      S.ensure_engine();local r,data=d:match("^([0-9A-F]+)|([0-9A-F]+)$")
      if not r or #r~=4 then return end
      local rev=tonumber(r,16);local g=S.unpack_state(data,now)
      if not g or rev<=S.last_revision or g.ack>S.seq then return end
      S.last_rx,S.last_revision=now,rev
      if S.view then
        if g.hp[2]<S.view.hp[2] then S.effect,S.effect_until="D",now+700
        elseif S.view.incoming[2]>0 and g.incoming[2]==0 and g.result==0 then S.effect,S.effect_until="B",now+700 end
      end
      S.view=g
      if S.pending and g.ack==S.pending.seq then S.feedback(g.reply,S.pending.spell);S.pending=nil end
      if S.phase=="joining" then S.phase="duel" end
      if g.result~=0 then S.phase,S.capture="result",nil end
    end
  end
end
