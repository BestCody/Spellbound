...
`python tools/memory_scenarios.py` is an optional, fuller regression probe using `lupa`'s 64-bit Lua 5.4 and the test badge mock. It exercises Home, first Teach, all three trained models, direct Duel entry, both feature stacks, an active match, and a later 4.4-second capture, then binary-searches the allocator cap for each stage. Install it with `python -m pip install lupa`. On v0.4.0, direct Duel entry requires 21,211 bytes more allocator cap than Home. The trained active-match stage requires 53,104 bytes more than Home and retains 76,908 bytes above the desktop harness. These are desktop regression references, **not** the badge's total Lua usage and do not prove the proposed 40 KiB device goal.
