...
`tests/design_pass.lua` adds 16 checks for radio-first startup, one-shot failure handling, one latch per frame, no catch-up bursts, shield feedback, pending acknowledgements, stale-effect cleanup, and widget bounds. A constrained production mock refuses Bluetooth startup after any app module has loaded, and a separate build-contract test locks the same Bluetooth-before-`app.lua` order. The design suite uses a simple local relay for presentation checks; the original integration suite remains the loss/reordering coverage.

`python tools/memory_scenarios.py` is a fuller regression probe using `lupa`'s 64-bit Lua 5.4 and the test badge mock. It exercises Home, first Teach, all three trained models, direct Duel entry, both feature stacks, an active match, and a later 4.4-second capture, then binary-searches the allocator cap for each stage. Install it with `python -m pip install lupa`. On v0.4.1, direct Duel entry requires 19,767 bytes more allocator cap than Home. The trained active-match stage requires 50,891 bytes more than Home and retains 74,020 bytes above the desktop harness. CI runs the probe with a 52,000-byte trained-match regression ceiling. These are desktop regression references, **not** the badge's total Lua usage and do not prove the proposed 40 KiB device goal.
