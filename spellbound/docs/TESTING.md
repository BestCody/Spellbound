...
`tests/design_pass.lua` adds 16 checks for lazy radio startup, one-shot failure handling, one latch per frame, no catch-up bursts, shield feedback, pending acknowledgements, stale-effect cleanup, and widget bounds. A separate build-contract test locks Bluetooth-before-Duel allocation order. The design suite uses a simple local relay for presentation checks; the original integration suite remains the loss/reordering coverage.

`python tools/memory_scenarios.py` is an optional, fuller regression probe using `lupa`'s 64-bit Lua 5.4 and the test badge mock. It exercises Home, first Teach, all three trained models, direct Duel entry, both feature stacks, an active match, and a later 4.4-second capture, then binary-searches the allocator cap for each stage. Install it with `python -m pip install lupa`. On v0.3.1, direct Duel entry required 21,195 bytes more allocator cap than Home. The trained active-match stage required 53,088 bytes more than Home and retained 76,934 bytes above the desktop harness. These are desktop regression deltas, **not** the badge's total Lua usage and do not prove the proposed 40 KiB device goal.
