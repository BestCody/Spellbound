...
The project lives in [`spellbound/`](spellbound/). Read the [project README](spellbound/README.md), [installation guide](spellbound/INSTALL.md), and [0.2.0 design pass](spellbound/docs/DESIGN_PASS.md). Use the lazy modular package in `spellbound/dist/app/`. `main.lua` is a tiny lifecycle bootstrap; feature code is split across `app.lua`, `core.lua`, `ui.lua`, `network.lua`, `casting.lua`, `gesture.lua`, and `engine.lua`.
