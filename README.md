...
The project lives in [`spellbound/`](spellbound/). Read the [project README](spellbound/README.md), [installation guide](spellbound/INSTALL.md), and [0.2.0 design pass](spellbound/docs/DESIGN_PASS.md). Use the low-memory modular package in `spellbound/dist/app/`. `main.lua` is a tiny lifecycle bootstrap; the physical build uses a 9-widget UI and 13 Lua files split so every lazy-loaded chunk stays at or below 4 KiB.
